...
function wgphpofficeMetaKeywords($content)
{
...
function wgphpofficeMetaDescription($content)
{
    global $xoopsTpl, $xoTheme;
    $myts = MyTextSanitizer::getInstance();
    $content = $myts->undoHtmlSpecialChars($myts->displayTarea($content));
 ...
 */
function wgphpoffice_RewriteUrl($module, $array, $type = 'content')
{
    $comment = '';
    $helper = \XoopsModules\Wgphpoffice\Helper::getInstance();
    $presentationsHandler = $helper->getHandler('presentations');
    ...
    if (isset($array['topic_alias']) && $array['topic_alias']) {
        ...
        $topic_name = wgphpoffice_Filter(xoops_getModuleOption('static_name', $module));
    }
 ...
 * @return string $url
 */
function wgphpoffice_Filter($url, $type = '') {
