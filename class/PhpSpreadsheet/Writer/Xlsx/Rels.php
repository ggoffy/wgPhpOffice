...
class Rels extends WriterPart
{
     ...
     * @return string XML Output
     */
    public function writeWorksheetRelationships(\XoopsModules\Wgphpoffice\PhpSpreadsheet\Worksheet\Worksheet $pWorksheet, $pWorksheetId = 1, $includeCharts = false)
    {
        // Create XML writer
        $objWriter = null;
        if ($this->getParentWriter()->getUseDiskCaching()) {
        ...
        if (($pWorksheet->getDrawingCollection()->count() > 0) || (count($charts) > 0) || $drawingOriginalIds) {
            ...
            // (! synchronize with \XoopsModules\Wgphpoffice\PhpSpreadsheet\Writer\Xlsx\Worksheet::writeDrawings)
    ...
    private function writeUnparsedRelationship(\XoopsModules\Wgphpoffice\PhpSpreadsheet\Worksheet\Worksheet $pWorksheet, XMLWriter $objWriter, $relationship, $type): void
    {
        $unparsedLoadedData = $pWorksheet->getParent()->getUnparsedLoadedData();
     ...
     * @return string XML Output
     */
    public function writeDrawingRelationships(\XoopsModules\Wgphpoffice\PhpSpreadsheet\Worksheet\Worksheet $pWorksheet, &$chartRef, $includeCharts = false)
    {
        // Create XML writer
        $objWriter = null;
        if ($this->getParentWriter()->getUseDiskCaching()) {
        ...
        while ($iterator->valid()) {
            if (
                $iterator->current() instanceof \XoopsModules\Wgphpoffice\PhpSpreadsheet\Worksheet\Drawing
                || $iterator->current() instanceof MemoryDrawing
            ) {
                // Write relationship for image drawing
                /** @var \XoopsModules\Wgphpoffice\PhpSpreadsheet\Worksheet\Drawing $drawing */
     ...
     * @return string XML Output
     */
    public function writeHeaderFooterDrawingRelationships(\XoopsModules\Wgphpoffice\PhpSpreadsheet\Worksheet\Worksheet $pWorksheet)
    {
        // Create XML writer
        $objWriter = null;
        if ($this->getParentWriter()->getUseDiskCaching()) {
    ...
    /**
     * @param $objWriter
     * @param \XoopsModules\Wgphpoffice\PhpSpreadsheet\Worksheet\Drawing $drawing
     * @param $i
     *
     * @return int
     */
